...
window.addEventListener('scroll', () => {
  ...
  if (y + 400 > skillsOffset) {
    ...
    percentages.forEach(
      (percentage) => (percentage.style.opacity = '1'),
    )
  }
  ...
  if (y + 500 < skillsOffset) {
    ...
    percentages.forEach(
      (percentage) => (percentage.style.opacity = '0'),
    )
  }
})
      ...
      if (project.getAttribute('data-id') === target) {
        ...
        project.addEventListener('mouseover', () => {
          ...
          project.style.boxShadow =
            '7px 12px 12px rgba(0, 0, 0, 0.12)'
        })

        project.addEventListener('mouseout', () => {
          project.style.transform = 'scale(1)'
          project.style.boxShadow =
            '5px 10px 10px rgba(0, 0, 0, 0.07)'
        })
      }
